...
class ProductionHardeningTests(unittest.TestCase):
    def test_no_secret_like_values_in_tracked_source(self):
        secret_patterns = [re.compile(r"(?:A[Ii]za[A-Za-z0-9_-]{20,}|sk-[A-Za-z0-9_-]{20,})"), re.compile(r"(?:api[_-]?key|authorization|bearer)\s*[:=]\s*['\"][^'\"]{12,}['\"]", re.I)]
        for path in REPO_ROOT.rglob("*"):
            if not path.is_file() or ".git" in path.parts or "__pycache__" in path.parts or path.suffix not in {".py", ".json", ".yml", ".yaml", ".md", ".txt"}:
                continue
    ...
    def test_canonical_team_contract(self):
        ...
        self.assertEqual({member.get("name") for member in team.get("members", [])}, {"researcher", "verifier", "market_analyst", "risk_analyst", "writer", "reviewer"})
    ...
    def test_artifact_manifest_is_complete_and_hashed(self):
        ...
        if not manifest.get("files_digest_sha256"):
            return
